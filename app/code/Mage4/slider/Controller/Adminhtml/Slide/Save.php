...
class Save extends \Magento\Backend\App\Action
{
    ...
    {
        return $this->_authorization->isAllowed('Mage4_Slider::slide_save');
    }
                ...
                foreach ($imageKeys as $imageKey) {
                    if (isset($_FILES[$imageKey]['name']) && $_FILES[$imageKey]['name']) {
                        ...
                        $result = $uploader->save(
                            $mediaDirectory->getAbsolutePath(\Mage4\Slider\Model\Slider::MEDIA_PATH)
                        );
                        // Resize images for use in srcSets
                        $model->prepareImagesForSrcset($result['path'] . $result['file']);

                        $model->setData($imageKey, \Mage4\Slider\Model\Slider::MEDIA_PATH . $result['file']);
